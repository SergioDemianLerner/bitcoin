...
bool ActivateBestChain(CValidationState& state, const CChainParams& chainparams, std::shared_ptr<const CBlock> pblock = std::shared_ptr<const CBlock>(),bool fCheckPOW=true);
...
/** Check whether 2Mb blocks are enabled for block. */
...
/** Check if the block consumes less or equal the maximum number of sigops. Returns true if block is valid regarding sigops */
